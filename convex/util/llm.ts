...
const DEMO_EMBEDDING_DIMENSION = 128;
const OPENAI_EMBEDDING_DIMENSION = 1536;
const TOGETHER_EMBEDDING_DIMENSION = 768;
const OLLAMA_EMBEDDING_DIMENSION = 1024;

export const EMBEDDING_DIMENSION: number = DEMO_EMBEDDING_DIMENSION;
...
export function getLLMConfig(): LLMConfig {
  ...
  if (process.env.LLM_API_URL) {
    ...
    const embeddingModel = process.env.LLM_EMBEDDING_MODEL ?? 'demo-local-embedding';
    return {
      provider: 'custom',
      url,
      chatModel,
...
export async function fetchEmbeddingBatch(texts: string[]) {
  const config = getLLMConfig();
  if (config.embeddingModel === 'demo-local-embedding') {
    return {
      demo: true as const,
      embeddings: texts.map(demoEmbedding),
      retries: 0,
      ms: 0,
    };
  }
...
function demoEmbedding(text: string): number[] {
  const values = Array.from({ length: EMBEDDING_DIMENSION }, () => 0);
  for (let i = 0; i < text.length; i++) {
    const slot = i % EMBEDDING_DIMENSION;
    values[slot] += ((text.charCodeAt(i) % 97) - 48) / 48;
  }
